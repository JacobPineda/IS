...
<body>
   ...
   <?php
		...
		*/
		...
		<form action = 'edit-distributor.php?id=$id' method='post'>
		<table>
        	<tr> 
	  			<td>Name</td>
                <td><input name='new_name' type='text'  value='".$name."' required></td>
			...
			<tr>
				<td><input  type='submit' name='edit_distributor' value='Save'/></td>
                <td><a class='btn' href='/IS/list/distributor.php'>Back</a></td>
		...
		if($_POST['edit_distributor']){
			...
			$qry = "SELECT * from Distributor where name = '{$new_name}'";
			$result = $conn->query($qry);
			$data = mysqli_fetch_array($result)['name'];

			if($data){
				echo "<center>Distributor name already exists!</center>" .generateForm($id, $curr_name);
			}else{
				if(!mysqli_query($conn, "UPDATE Distributor SET name = '{$new_name}'
					WHERE dist_no = '{$id}'")){
